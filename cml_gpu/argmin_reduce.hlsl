             ...
             uint3 p_gtid : SV_GroupThreadID, uint p_gi : SV_GroupIndex)
{
    ...
    if (l_y < l_shape_tensor[1].y && l_z < l_shape_tensor[1].z && l_w < l_shape_tensor[1].w)
    {
        ...
        float l_min_value = asfloat(l_tensors.Load(l_byte_offset_tensor[0] + 4 * l_idx0));
        uint l_min_index = 0;

        for (uint l_i = 1; l_i < l_idx_end; l_i++)
        {
        ...
        l_tensors.Store(l_byte_offset_tensor[1] + 4 * l_idx_out, asuint(l_min_index));
    }
}
