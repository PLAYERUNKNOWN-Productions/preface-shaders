...
#include "avg_pool_func.hlsl"

// uint m_tensor_count;    // 2
// uint m_tensor_offset_0; // Input
// uint m_tensor_offset_1; // Output

uint get_attribs(in ByteAddressBuffer p_attrib_buffer, in uint p_byte_offset, out avg_pool_attribs_s p_attribs)
{
    ...
    if (l_k2 < l_out_ch && l_i2 < l_out_shape[3] && l_i1 < l_out_shape[2])
    {
        ...
        for (uint l_j1 = 0; l_j1 < l_d_f; l_j1++)
        {
            for (uint l_j2 = 0; l_j2 < l_d_f; l_j2++)
            {
                ...
                int l_idx_input = l_ind3 + l_ind2 * l_dim_1 + l_k2 * l_dim_0 * l_dim_1;
        ...
        l_avg_value /= (l_d_f * l_d_f);

        l_tensors.Store(l_out_byte_offset + 4 * l_idx_output, asuint(l_avg_value));
    }
}
