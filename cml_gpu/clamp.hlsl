             ...
             uint3 p_gtid : SV_GroupThreadID, uint p_gi : SV_GroupIndex)
{
    ...
    if (l_id0 < l_out_shape_output[0] && l_id1 < l_out_shape_output[1] && l_id2 < l_out_shape_output[2] && l_id3 < l_out_shape_output[3])
    {
        ...
        float l_low = asfloat(l_tensors.Load(l_in_byte_offset_low + 4 * l_idx_x));
        float l_high = asfloat(l_tensors.Load(l_in_byte_offset_high + 4 * l_idx_y));
        float l_z = asfloat(l_tensors.Load(l_in_byte_offset_input + 4 * l_idx_z));

        l_z = l_high < l_z ? l_high : l_z;
        l_z = l_low > l_z ? l_low : l_z;

        // Store result
        l_tensors.Store(l_out_byte_offset_output + 4 * l_idx_z, asuint(l_z));
    }
}
