...
struct conv_leaky_relu_attribs_s
{
    int4 m_padding;
    int2 m_stride;
    int2 m_dilation;
...
uint get_attribs(in ByteAddressBuffer p_attrib_buffer, in uint p_byte_offset, out conv_leaky_relu_attribs_s p_attribs)
{
...
void conv_leaky_relu_func(uint3 p_gid, uint3 p_dtid, uint3 p_gtid, uint p_gi)
{
    CML_GET_BUFFERS;
    ...
    // Get the attributes
    conv_leaky_relu_attribs_s l_attribs;
    ...
    if (l_i2 < l_out_shape[3] && l_i1 < l_out_shape[2])
    {
        ...
        l_out = (l_out > 0.0) ? l_out : l_attribs.m_alpha * l_out;
