...
uint get_attribs(in ByteAddressBuffer p_attrib_buffer, in uint p_byte_offset, out deconv_a_attribs_s p_attribs)
{
...
#define l_k2 (p_gtid.x)

#define l_idx_output (l_i2 + l_i1 * l_d_o + l_k1 * l_d_o * l_d_o)
groupshared float l_sum[512];

void deconv_a_func(uint3 p_gid, uint3 p_dtid, uint3 p_gtid, uint p_gi)
{
    CML_GET_BUFFERS;
    ...
    if (l_k2 < l_in_ch)
    {
        int l_idx_input0 = l_k2 * l_d_i * l_d_i;
        ...
        for (uint l_j1 = 0; l_j1 < l_d_f; l_j1++)
        {
            ...
            int l_idx_input1 = l_idx_input0 + l_ind2 * l_d_i;
            uint l_cond1 = (l_a_ind2 == 0) && (l_ind2 >= 0) && (l_ind2 < l_d_i);
            ...
            for (uint l_j2 = 0; l_j2 < l_d_f; l_j2++)
            {
                ...
                int l_idx_input = l_idx_input1 + l_ind3;

                float l_t1 = asfloat(l_tensors.Load(l_in_byte_offset_input + 4 * (uint)l_idx_input));
                float l_t2 = l_filter[l_j2];

                float l_cond = (float)(l_cond1 && l_a_ind3 == 0 && l_ind3 >= 0 && l_ind3 < l_d_i);
                l_temp += l_t1 * l_t2 * l_cond;
            }
        }
    }
    ...
    uint l_a = (uint)(l_in_ch / 16.0 - 0.0001) + 1;
    if (l_k2 < l_a)
    {
        l_temp = 0;
        for (uint l_i = 0; l_i < 16; l_i++)
        {
            l_temp += l_sum[l_k2 * 16 + l_i];
        }
    }
