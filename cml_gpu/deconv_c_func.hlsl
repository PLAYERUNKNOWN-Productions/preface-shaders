...
uint get_attribs(in ByteAddressBuffer p_attrib_buffer, in uint p_byte_offset, out deconv_c_attribs_s p_attribs)
{
...
#define l_k1 (p_gid.z)
#define l_i1 (p_gid.y * GROUP_SIZE_Y + p_gtid.y)
#define l_i2 (p_gid.x * GROUP_SIZE_X + p_gtid.x)
#define l_idx_output (l_i2 + l_i1 * l_d_o + l_k1 * l_d_o * l_d_o)

groupshared float l_sum[GROUP_SIZE_X*GROUP_SIZE_X*GROUP_SIZE_Z];

void deconv_c_func(uint3 p_gid, uint3 p_dtid, uint3 p_gtid, uint p_gi)
{
    CML_GET_BUFFERS;
    ...
    uint l_idx_filter0 = l_k1 * l_d_f * l_d_f;
    if (l_i2 < l_out_shape[3] && l_i1 < l_out_shape[2])
    {
        for (uint l_k2 = p_gtid.z; l_k2 < l_in_ch; l_k2 += GROUP_SIZE_Z)
        {
            uint l_idx_filter1 = l_idx_filter0 + l_k2 * l_d_f * l_d_f * l_out_ch;
            ...
            for (uint l_j1 = 0; l_j1 < l_d_f; l_j1++)
            {
                ...
                if (l_a_ind2 == 0 && l_ind2 >= 0 && l_ind2 < l_d_i) // ARRAY
                {
                    uint l_idx_filter = l_j1 * l_d_f + l_idx_filter1;
                    float4 l_filter = asfloat(l_tensors.Load4(l_in_byte_offset_filter + 4 * l_idx_filter));

                    int l_idx_input1 = l_ind2 * l_d_i + l_idx_input0;
                    ...
                    for (uint l_j2 = 0; l_j2 < l_d_f; l_j2++)
                    {
                        ...
                        {
                            int l_idx_input = l_ind3 + l_idx_input1;
        ...
        l_sum[p_gtid.z * GROUP_SIZE_X * GROUP_SIZE_Y + p_gtid.y * GROUP_SIZE_X + p_gtid.x] = l_temp;
    }
    ...
    {
        l_temp = 0;
        for(uint l_iii=0;l_iii< GROUP_SIZE_Z;l_iii++)
        {
            l_temp += l_sum[l_iii * GROUP_SIZE_X * GROUP_SIZE_Y + p_gtid.y * GROUP_SIZE_X + p_gtid.x];
        }
