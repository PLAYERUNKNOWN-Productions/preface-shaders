...
uint get_attribs(in ByteAddressBuffer p_attrib_buffer, in uint p_byte_offset, out deconv_attribs_s p_attribs)
{
...
void deconv_func(uint3 p_gid, uint3 p_dtid, uint3 p_gtid, uint p_gi)
{
    CML_GET_BUFFERS;
    ...
    if (l_i2 < l_out_shape[3] && l_i1 < l_out_shape[2])
    {
        for (uint l_k2 = 0; l_k2 < l_in_ch; l_k2++)
        {
            ...
            for (uint l_j1 = 0; l_j1 < l_d_f; l_j1++)
            {
                ...
                uint l_cond = l_a_ind2 == 0 && l_ind2 >= 0 && l_ind2 < l_d_i;

                if (l_cond)
                {
                    for (uint l_j2 = 0; l_j2 < l_d_f; l_j2++)
                    {
                        int l_a_ind3 = l_i2 + l_p2 - l_j2;
                        int l_ind3 = l_a_ind3 / l_s2;

                        l_a_ind3 -= l_ind3 * l_s2;

                        float l_t1 = asfloat(l_tensors.Load(l_in_byte_offset_input + 4 * (uint)(l_idx_input1 + l_ind3)));
                        float l_t2 = asfloat(l_tensors.Load(l_in_byte_offset_filter + 4 * (l_idx_filter1 + l_j2)));

                        l_cond = l_a_ind3 == 0 && l_ind3 >= 0 && l_ind3 < l_d_i;
