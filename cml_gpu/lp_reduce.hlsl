             ...
             uint3 p_gtid : SV_GroupThreadID, uint p_gi : SV_GroupIndex)
{
    ...
    float l_p = asfloat(l_attributes.Load(l_meta_data.m_attrib_offset + 8));
    ...
    if (l_y < l_shape_tensor[1].y && l_z < l_shape_tensor[1].z && l_w < l_shape_tensor[1].w)
    {
        ...
        int l_idx_out = (1 - l_axes[1]) * l_y * l_shape_tensor[1].w * l_shape_tensor[1].z + (1 - l_axes[2]) * l_z * l_shape_tensor[1].w + (1 - l_axes[3]) * l_w;
        ...
        for (uint l_i = 0; l_i < l_idx_end; l_i++)
        {
            ...
            l_sum += pow(abs(l_value), l_p);
        }

        l_sum = pow(l_sum, 1.0 / l_p);

        l_tensors.Store(l_byte_offset_tensor[1] + 4 * l_idx_out, asuint(l_sum));
    }
}
