...
uint get_attribs(in ByteAddressBuffer p_attrib_buffer, in uint p_byte_offset, out median_pool_attribs_s p_attribs)
{
...
#define FLOAT_SIZE 4
#define FILTER_SIZE 9 // 3x3
#define MEDIAN_IND 4  // 9/2
    ...
    uint3 p_gtid : SV_GroupThreadID, uint p_gi : SV_GroupIndex)
{
    ...
    // Get the attributes
    median_pool_attribs_s l_attribs;
    ...
    if (l_k2 < l_out_ch && l_i2 < l_out_shape[3] && l_i1 < l_out_shape[2])
    {
        ...
        float l_temp_array[FILTER_SIZE];
        uint l_count = 0;
        ...
        for (uint l_j1 = 0; l_j1 < l_d_f; l_j1++)
        {
            for (uint l_j2 = 0; l_j2 < l_d_f; l_j2++)
            {
                ...
                else if (l_ind2 >= l_d_i)
                {
                    l_ind2 -= (l_d_i - 1);
                    l_ind2 = l_d_i - 1 - l_ind2;
                }
                ...
                else if (l_ind3 >= l_d_i)
                {
                    l_ind3 -= (l_d_i - 1);
                    l_ind3 = l_d_i - 1 - l_ind3;
                }

                uint l_idx_input = l_ind3 + l_ind2 * l_dim_1 + l_k2 * l_dim_1 * l_dim_1;
                l_temp_array[l_count++] = asfloat(l_tensors.Load(l_in_byte_offset_input + FLOAT_SIZE * l_idx_input));
            }
        }
        ...
        // Choose 5th largest element (=median)
        for (uint l_k = 0; l_k <= MEDIAN_IND; l_k++)
        {
            l_min = FLT_MAX;
            l_index = 0;

            for (uint l_i = 0; l_i < FILTER_SIZE; l_i++)
            {
                if (l_temp_array[l_i] < l_min)
                {
                    l_min = l_temp_array[l_i];
                    l_index = l_i;
                }
            }
            l_temp_array[l_index] = 9999999999999.9;
        }
        
        l_tensors.Store(l_out_byte_offset + FLOAT_SIZE * l_idx_output, asuint(l_min));
    }
}
