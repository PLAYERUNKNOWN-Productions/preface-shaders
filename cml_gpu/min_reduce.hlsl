        ...
        for (uint l_i = 0; l_i < l_idx_end_1; l_i++)
        {
            for (uint l_j = 0; l_j < l_idx_end_2; l_j++)
            {
                for (uint l_k = 0; l_k < l_idx_end_3; l_k++)
                {
                    ...
                    uint l_cond = l_value < l_min_value;
                    l_min_value = l_cond * l_value + (1 - l_cond) * l_min_value;
                }
            }
        }

        l_tensors.Store(l_byte_offset_tensor[1] + 4 * l_idx_out, asuint(l_min_value));
    }
}
