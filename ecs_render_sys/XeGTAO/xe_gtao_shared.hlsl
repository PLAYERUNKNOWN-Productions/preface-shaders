...
namespace XeGTAO
{
    ...
    struct GTAOConstants
    {
        ...
        uint                    OutWorkingDepthMIP1; // UAV: output viewspace depth MIP (these are views into SrcWorkingDepth MIP levels)
        uint                    OutWorkingDepthMIP2; // UAV: output viewspace depth MIP (these are views into SrcWorkingDepth MIP levels)
        uint                    OutWorkingDepthMIP3; // UAV: output viewspace depth MIP (these are views into SrcWorkingDepth MIP levels)
        uint                    OutWorkingDepthMIP4; // UAV: output viewspace depth MIP (these are views into SrcWorkingDepth MIP levels)
        ...
        uint                    OutFinalAOTerm;      // UAV: final AO term - just 'visibility' or 'visibility + bent normals'
    };
    ...
    // NOTE: Changed default to 0 as there's no measurable performance difference and then we don't need two sets of shaders
