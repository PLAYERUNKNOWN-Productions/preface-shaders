...
void cs_memset_texture(uint3 p_dispatch_thread_id : SV_DispatchThreadID)
{
...
[numthreads(MB_MEMSET_UAV_THREADGROUP_SIZE_X * MB_MEMSET_UAV_THREADGROUP_SIZE_Y, 1, 1)]
void cs_memset_buffer_uint_zero(uint3 p_dispatch_thread_id : SV_DispatchThreadID)
{
    RWStructuredBuffer<uint> l_buffer = ResourceDescriptorHeap[g_push_constants.m_uav];
    l_buffer[p_dispatch_thread_id.x] = 0;
}
