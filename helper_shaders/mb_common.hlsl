...
//-----------------------------------------------------------------------------
// Visibility Buffering helper functions
//-----------------------------------------------------------------------------
...
#else
    float4 l_color = l_texture.Sample(p_sampler, p_uv, 0, l_min_mip_level);
#endif
    return l_color;
}
...
//-----------------------------------------------------------------------------
// Remap uv for render scale
float2 get_remapped_uv(float2 p_uv, float2 p_render_scale)
{
    float2 l_remapped_uv = p_uv * p_render_scale;
    return l_remapped_uv;
}
